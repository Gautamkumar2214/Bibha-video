...
bibha video is a Javascript tool to easily create beautiful, simple, mobile-friendly, vertical timelines.

[![Build Status](https://travis-ci.org/datanews/bibha-video.svg?branch=master)](https://travis-ci.org/datanews/bibha-video) [![Sauce Browser Test Status](https://saucelabs.com/buildstatus/bibha-video)](https://saucelabs.com/u/bibha-video)

## Install

There are many ways to get the code.

* Use [Bower](https://bower.io): `bower install bibha-video`
* Use [npm](https://www.npmjs.com/): `npm install bibha-video`
* If you aren't familiar with those technologies, just [download](https://github.com/datanews/bibha-video/archive/0.1.1.zip) the code directly from Github and separately download the dependencies, [Underscore](http://underscorejs.org/) and [Moment.js](http://momentjs.com/docs/).
...
bibha video is a Javascript file and a CSS file.  You can use RequireJS, AMD, Browserify, or you can manually include the files below like the following:
    ...
    <!-- bibha video -->
    <link href="dist/bibha-video.css" rel="stylesheet" type="text/css">
...
Or, the full CDN version:
    ...
    <link href="https://cdn.rawgit.com/datanews/bibha-video/0.1.1/dist/bibha-video.min.css" rel="stylesheet" type="text/css">
    <script src="https://cdn.rawgit.com/datanews/bibha-video/0.1.1/dist/bibha-video.min.js"></script>

## Use

bibha video is simply a object your create and give it configuration:

    var t = new bibhavideo({
      el: 'example-bibha-video-container',
...
You can use the following options when defining a bibha video timeline:

* `el`: The ID string of the DOM element to put your bibha video container in.  This is required.  You can also use CSS selectors if you want.
...
* `keyMapping`: If you have entry data that is keyed differently, you can provide a basic object to convert when it is processed.  For instance:  
    ```
  {
    ...
    ```
* `template`: If you want to override the HTML output of the timeline, use your own template.  See the `src/bibha-video.tpl.html` for a starting point.  You can provide a string that will be processed with [Underscore's template function](http://underscorejs.org/#template), or provide your own templating function.  The function will be passed the `groups` of entries, `_` (Underscore), the timeline `title`, and the whole `bibhavideo` object.
...
1. Get the code (replace with your fork's repository URL) and enter into the code directory: `git clone https://github.com/datanews/bibha-video.git && cd bibha-video`
...
The following are the necessary steps for creating a new version of bibha video.
